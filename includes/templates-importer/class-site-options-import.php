 ...
 *
 */
class Mageewp_Site_Options_Import {

	/**
	 * Instance of Mageewp_Site_Options_Importer
	 */
	private static $_instance = null;

	/**
	 * Instanciate Mageewp_Site_Options_Importer
	 */
		...
		if ( isset( $cats ) ) {
			...
			foreach ( $cats as $key => $cat ) {
				...
				if ( ! empty( $cat['slug'] ) && ! empty( $cat['thumbnail_src'] ) ) {
					...
					$image = (object) Mageewp_Sites_Helper::_sideload_image( $cat['thumbnail_src'] );
	...
	private function insert_logo( $image_url = '' ) {
		...
		$data = (object) Mageewp_Sites_Helper::_sideload_image( $image_url );

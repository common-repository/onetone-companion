<?php
if(!class_exists('OnetonePosts_Widget')){
 class OnetonePosts_Widget extends WP_Widget {
    
	public function __construct() {
		
		parent::__construct(
			'onetone_recent_posts', // Base ID
			__( 'Onetone: Recent Posts', 'onetone-companion' ), // Name
			array( 'description' => __( 'Recent Posts.', 'onetone-companion' ), ) // Args
		);
		
	}
 	function form( $instance ) {
 	    $defaults = array('list_num' => '4','style' => '1', 'title' => __( 'Recent Posts', 'onetone-companion' ));
...
<p>
  <label for="<?php echo $this->get_field_id( 'title' ); ?>">
    <?php _e('Title', 'onetone-companion'); ?>
    :</label>
  <br />
...
<p>
  <label for="<?php echo $this->get_field_id( 'list_num' ); ?>">
    <?php _e('Recent Posts List Num', 'onetone-companion'); ?>
    :</label>
  <br />
...
<?php
	...
	function widget( $args, $instance ) {
 		extract( $args );
 	    $title    = apply_filters(__('Recent Posts', 'onetone-companion'), esc_attr($instance['title']) );
...
<div class="widget-project">
  <div class="row">
   ...
   if ( has_post_thumbnail() ) {
         $featured_image = wp_get_attachment_image_src(get_post_thumbnail_id(), 'onetone-widget-post-image');
		 $source = get_site_url();
		 if($featured_image[0] !=""){
...
function onetone_companion_recent_posts(){
	
	register_widget('OnetonePosts_Widget');
						
}
add_action( 'widgets_init', 'onetone_companion_recent_posts' );
